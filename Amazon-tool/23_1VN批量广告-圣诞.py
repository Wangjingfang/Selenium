...
def table_info_file(file_path):
    ...
    max_bid_func = lambda x:0.04 if x == 'US' or x == 'CA' else(0.04 if x == 'UK' or x == 'DE' or x == 'FR' or x == 'IT' or x == 'ES' else(1 if x == 'IN' else (5 if x == 'JP' else (0.15 if x == 'AU' else(0.24 if x == 'AE' else 0.5)))))
    ...
    for sta in station:
        ...
        out_path = str(shop) + '-' + str(report_sequence) + '-' + country  + '-圣诞' + '.xlsx'  
...
if __name__ == '__main__':
    ...
    if not os.path.exists('圣诞-1VN已分解文件'):
        os.mkdir('圣诞-1VN已分解文件')
    
    os.chdir('圣诞-1VN已分解文件')
