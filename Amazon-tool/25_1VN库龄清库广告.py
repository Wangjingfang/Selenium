...
def table_info_file(file_path):
    ...
    cam_budget_func = lambda x:10 if x == 'US' or x == 'CA' or x == 'MX' or x == 'UK' or x == 'DE' or x == 'FR' or x == 'IT' or x == 'ES' or x == 'AU' or x == 'AE' else(500 if x == 'IN' else 300)
    
    max_bid_func = lambda x:0.1 if x == 'US' or x == 'CA' else(0.08 if x == 'UK' or x == 'DE' or x == 'FR' or x == 'IT' or x == 'ES' else(2 if x == 'IN' else (5 if x == 'JP' else (0.12 if x == 'AU' else(0.24 if x == 'AE' else 0.7)))))
    ...
    for sta in station:
        ...
        out_path = str(shop) + '-' + str(report_sequence) + '-' + country  + '-库龄清库' + '.xlsx'  
...
if __name__ == '__main__':
    ...
    if not os.path.exists('库龄清库-1VN已分解文件'):
        os.mkdir('库龄清库-1VN已分解文件')
    
    os.chdir('库龄清库-1VN已分解文件')
