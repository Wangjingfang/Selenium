...
def table_info_file(file_path):
    ...
    cam_budget_func = lambda x:15 if x == 'US' or x == 'CA' or x == 'MX' or x == 'UK' or x == 'DE' or x == 'FR' or x == 'IT' or x == 'ES' or x == 'AU' or x == 'AE' else(5000 if x == 'IN' else 3000)
    
    max_bid_func = lambda x:0.08 if x == 'US' or x == 'CA' else(0.05 if x == 'UK' or x == 'DE' or x == 'FR' or x == 'IT' or x == 'ES' else(1 if x == 'IN' else (5 if x == 'JP' else (0.15 if x == 'AU' else(0.28 if x == 'AE' else 0.5)))))
    ...
    for sta in station:
        ...
        out_path = str(shop) + '-' + str(report_sequence) + '-' + country  + '-节日促销清库' + '.xlsx'  
...
if __name__ == '__main__':
    ...
    if not os.path.exists('节日促销清库-1VN已分解文件'):
        os.mkdir('节日促销清库-1VN已分解文件')
    
    os.chdir('节日促销清库-1VN已分解文件')
