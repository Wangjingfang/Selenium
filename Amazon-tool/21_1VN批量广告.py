...
update20201023:新增SA站点，模板同US，价格竞价同AE
...
def table_info_file(file_path):
    ...
    date = datetime.datetime.today() + datetime.timedelta(days=2)  #确定报告中时间是否需要延迟几天，默认为0，即不需要延迟
    ...
    max_bid_func = lambda x:0.03 if x == 'US' or x == 'CA' else(0.02 if x == 'UK' or x == 'DE' or x == 'FR' or x == 'IT' or x == 'ES' else(1 if x == 'IN' else (2 if x == 'JP' else (0.1 if x == 'AU' else(0.24 if x == 'AE' else 0.35)))))
    ...
    for sta in station:
        ...
        cam_name = sta_short + '-' + str(datetime.datetime.today().strftime('%Y%m')) # + '_alllisting'   #自此以后采用年月来命名广告活动不再以年月日方式
        
        cam_budget = cam_budget_func(sta[-2:])
        
        max_bid = max_bid_func(sta[-2:])
        
        cam_start_date =  cam_start_date_func(sta[-2:])   
        ...
        out_path = str(shop) + '-' + str(report_sequence) + '-' + country  + '-FBM' + '.xlsx'  
...
if __name__ == '__main__':
    ...
    print('\n请定位到338行查看广告活动名称是否按要求;\n')
    print('请输入待分解生成1VN文件（158系统中导出的原始文件）')
    print('若需要更改报告内的广告活动时间，请定位到324行；')
    path = input('excel文件：')
    if '"' in path:
        path = path.replace('"','')
    ...
    if not os.path.exists('1VN已分解文件'):
        os.mkdir('1VN已分解文件')
