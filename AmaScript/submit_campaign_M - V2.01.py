...
@2021/05/06 更新
1. 增加日本站的手动上传
2. 修改了data_to_campaign()函数，从内部调整广告活动预算和广告组竞价
"""
...
def main():
    ...
    #不同组的竞价增幅倍率
    '''
    :param:asin_up  asin的竞价提高倍数
    :param:broad_up broad的竞价提高倍数  2021/02/24:1     2021/6/10 : 0.9
    :param:phrase_up phrase的竞价提高倍数
    :param:exact_up exact的竞价提高倍数
    '''
    asin_up = 1
    broad_up = 0.9
    phrase_up = 1.1
    ...
    base_bid_us_ca = 0.1
    base_bid_uk_de = 0.09
    base_bid_fr_it_es = 0.06
    base_bid_jp = 7.00

    keywords_data = read_excel(path,data_to_save,data_to_current,base_acos,base_cr,base_order,phrase_cr,phrase_order,exact_cr,exact_order,jp_base_acos,jp_base_cr,jp_base_order,jp_phrase_cr,jp_phrase_order,jp_exact_cr,jp_exact_order)

    data_to_campaign(keywords_data,asin_up,broad_up,phrase_up,exact_up,base_bid_us_ca,base_bid_uk_de,base_bid_fr_it_es,base_bid_jp)

#读取tableau导出的数据信息
def read_excel(path,data_to_save,data_to_current,base_acos,base_cr,base_order,phrase_cr,phrase_order,exact_cr,exact_order,jp_base_acos,jp_base_cr,jp_base_order,jp_phrase_cr,jp_phrase_order,jp_exact_cr,jp_exact_order):

    try:
        df = pd.read_excel(path, sheet_name = 0)

        # Step1：获取搜索词数据，搜索词默认Broad，待判断Phrase、Exact
        keywords_step1 = df[['Station','Country','Campaign Name_M','SellSKU','Search Term Type', 'Customer Search Terms', 'Bid', "CR", "Orders", "Acos"]]
        ...
        keywords_step1_ASIN_jp = keywords_step1[(keywords_step1["Search Term Type"] == "ASIN") & (keywords_step1["Country"] == "JP") & (keywords_step1.Orders >= jp_base_order) & (keywords_step1.CR >= jp_base_cr) & (keywords_step1.Acos <= jp_base_acos)]

        # 合并符合条件的数据表和不符合条件的数据表
        keywords_step1_ASIN_total_concat = pd.concat([keywords_step1_ASIN, keywords_step1_ASIN_jp,keywords_step1_ASIN_total], axis=0)
        # 去重，默认drop_duplicates() ，keep='first',保留第一次出现的重复行数据，删除后面的重复行数据；默认值inplace='false'，不对原表进行修改
        keywords_step1_ASIN_total_concat = keywords_step1_ASIN_total_concat.drop_duplicates()
        # 计算ASIN_pending的行
        keywords_step1_ASIN_pending = keywords_step1_ASIN_total_concat[(keywords_step1_ASIN.shape[0] + keywords_step1_ASIN_jp.shape[0]):]
        ...
        keywords_step1_Broad_jp = keywords_step1[(keywords_step1["Search Term Type"] == "Broad") & (keywords_step1["Country"] == "JP") & (keywords_step1.Orders >= jp_base_order) & (keywords_step1.CR >= jp_base_cr) & (keywords_step1.Acos <= jp_base_acos)]

        keywords_step1_Broad_total_concat = pd.concat([keywords_step1_Broad, keywords_step1_Broad_jp, keywords_step1_Broad_total], axis=0)
        keywords_step1_Broad_total_concat = keywords_step1_Broad_total_concat.drop_duplicates()
        keywords_step1_Broad_pending = keywords_step1_Broad_total_concat[(keywords_step1_Broad.shape[0] + keywords_step1_Broad_jp.shape[0]):]
        ...
        keywords_step1_Phrase_jp = keywords_step1[(keywords_step1["Search Term Type"] == "Broad") & (keywords_step1["Country"] == "JP") & (keywords_step1.Orders >= jp_phrase_order) & (keywords_step1.CR >= jp_phrase_cr) & (keywords_step1.Acos <= jp_base_acos)]
        keywords_step1_Phrase = pd.concat([keywords_step1_Phrase,keywords_step1_Phrase_jp], axis=0)
        ...
        keywords_step1_Exact_jp = keywords_step1[(keywords_step1["Search Term Type"] == "Broad") & (keywords_step1["Country"] == "JP") & (keywords_step1.Orders >= jp_exact_order) & (keywords_step1.CR >= jp_exact_cr) & (keywords_step1.Acos <= base_acos)]
        keywords_step1_Exact = pd.concat([keywords_step1_Exact, keywords_step1_Exact_jp], axis=0)
...
def data_to_campaign(keywords_data, asin_up,broad_up,phrase_up,exact_up,base_bid_us_ca,base_bid_uk_de,base_bid_fr_it_es,base_bid_jp):
        ...
        for station in station_set:
            ...
            for i in Campaign_Name_M:
                ...
                for y in [keywords_step1_ASIN_pending, keywords_step1_Broad_pending]:
                    if station[-2:] in ["JP"]:
                        y["Bid"] = y["Bid"].map(lambda x: base_bid_jp if round(x * broad_up,2) > base_bid_jp else round(x * broad_up,2))
                    elif station[-2:] in ["US", "CA"]:
                        ...
                        # decimal.Decimal(x * broad_up).quantize(decimal.Decimal("0.01")) > base_bid_us_ca else decimal.Decimal(x * broad_up).quantize(decimal.Decimal("0.01"))
                        # y["Bid"] = y["Bid"].map(lambda x: base_bid_uk_de if round(x * broad_up, 2) > base_bid_uk_de else round(x * broad_up, 2))
                #正常四组设置竞价增幅
